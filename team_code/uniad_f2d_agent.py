"""Fail2Drive adapter for Bench2DriveZoo's UniAD closed-loop agent."""
...
# Bench2DriveZoo's UniAD module reads this at import time. Keep it truthy so its
# config-name parsing follows the Bench2Drive closed-loop format.
os.environ.setdefault("IS_BENCH2DRIVE", "1")

from Bench2DriveZoo.team_code.uniad_b2d_agent import UniadAgent as Bench2DriveUniadAgent  # noqa: E402
from srunner.scenariomanager.carla_data_provider import CarlaDataProvider  # noqa: E402


def get_entry_point():
    return "Fail2DriveUniadAgent"
...
class Fail2DriveUniadAgent(Bench2DriveUniadAgent):
    """Adapts Bench2DriveZoo UniAD to fail2drive's local leaderboard API."""
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        if len(parts) < 2:
            raise ValueError("UniAD agent config must be '<config.py>+<checkpoint.pth>[+save_name]'.")
        if len(parts) == 2:
            path_to_conf_file = path_to_conf_file + "+uniad_f2d"
    ...
    def sensors(self):
        ...
        if _strtobool(os.environ.get("F2D_UNIAD_USE_BEV_SENSOR", "0")):
            return sensors
